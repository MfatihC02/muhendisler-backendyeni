...
StockReservationSchema.methods.confirm = async function (session) {
    // CHECKOUT veya CONFIRMED durumundaki rezervasyonlar onaylanabilir
    if (!['CHECKOUT', 'CONFIRMED'].includes(this.status)) {
        throw new Error('Sadece CHECKOUT veya CONFIRMED durumundaki rezervasyonlar onaylanabilir');
    }
